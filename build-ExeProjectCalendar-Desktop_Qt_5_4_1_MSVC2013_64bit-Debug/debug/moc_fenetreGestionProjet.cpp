/****************************************************************************
** Meta object code from reading C++ file 'fenetreGestionProjet.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.4.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../ExeProjectCalendar/fenetreGestionProjet.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'fenetreGestionProjet.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.4.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Gestionprojets_t {
    QByteArrayData data[5];
    char stringdata[58];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Gestionprojets_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Gestionprojets_t qt_meta_stringdata_Gestionprojets = {
    {
QT_MOC_LITERAL(0, 0, 14), // "Gestionprojets"
QT_MOC_LITERAL(1, 15, 13), // "chargerProjet"
QT_MOC_LITERAL(2, 29, 0), // ""
QT_MOC_LITERAL(3, 30, 12), // "ouvrirProjet"
QT_MOC_LITERAL(4, 43, 14) // "supprimeProjet"

    },
    "Gestionprojets\0chargerProjet\0\0"
    "ouvrirProjet\0supprimeProjet"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Gestionprojets[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   29,    2, 0x0a /* Public */,
       3,    0,   30,    2, 0x0a /* Public */,
       4,    0,   31,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void Gestionprojets::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Gestionprojets *_t = static_cast<Gestionprojets *>(_o);
        switch (_id) {
        case 0: _t->chargerProjet(); break;
        case 1: _t->ouvrirProjet(); break;
        case 2: _t->supprimeProjet(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject Gestionprojets::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Gestionprojets.data,
      qt_meta_data_Gestionprojets,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *Gestionprojets::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Gestionprojets::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_Gestionprojets.stringdata))
        return static_cast<void*>(const_cast< Gestionprojets*>(this));
    return QWidget::qt_metacast(_clname);
}

int Gestionprojets::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}
struct qt_meta_stringdata_creationProjet_t {
    QByteArrayData data[1];
    char stringdata[15];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_creationProjet_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_creationProjet_t qt_meta_stringdata_creationProjet = {
    {
QT_MOC_LITERAL(0, 0, 14) // "creationProjet"

    },
    "creationProjet"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_creationProjet[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       0,    0, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

       0        // eod
};

void creationProjet::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    Q_UNUSED(_o);
    Q_UNUSED(_id);
    Q_UNUSED(_c);
    Q_UNUSED(_a);
}

const QMetaObject creationProjet::staticMetaObject = {
    { &Gestionprojets::staticMetaObject, qt_meta_stringdata_creationProjet.data,
      qt_meta_data_creationProjet,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *creationProjet::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *creationProjet::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_creationProjet.stringdata))
        return static_cast<void*>(const_cast< creationProjet*>(this));
    return Gestionprojets::qt_metacast(_clname);
}

int creationProjet::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = Gestionprojets::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    return _id;
}
QT_END_MOC_NAMESPACE
